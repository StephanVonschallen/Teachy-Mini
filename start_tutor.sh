#!/bin/bash
# Start Reachy Mini Tutor (Simulation)
# Usage: ./start_tutor.sh [--no-camera] [--profile tutor_buddy]
...
osascript -e 'tell app "Terminal" to do script "cd /Users/karimkrimo/Projects/reachy_mini/reachy_mini_conversation_app && uv run mjpython $(uv run which reachy-mini-daemon) --sim --scene minimal --no-localhost-only"'
